...
function RecommendedProducts({category,heading}) {
  ...
  return (
    <div className='md:container mx-auto px-4 py-2 my-6 '>
          <h1 className='text-4xl font-semibold py-4'>{heading}</h1>
         <div className='grid grid-cols-[repeat(auto-fit,minmax(110px 110px))] md:grid-cols-[repeat(auto-fit,minmax(420px,420px))] md:justify-between'>
            ...
            HozData.map((product)=>{
                return(
                    ...
                    <div className='bg-blue-50 p-3 min-w-[250px]'> 
                        ...
                        <button className='bg-black  bg-opacity-80 p-4 w-full mt-2 text-white rounded-l-xl rounded-r-xl text-xl' onClick={(e)=>handleCart(e,product._id)}>Add To Cart</button>
                    </div> 
                 </Link>
                )
            })
