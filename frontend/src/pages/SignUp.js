 ...
 const navigate=useNavigate()
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
     ...
     data.name.length<Number(8) ?
      seterr((prev)=>{
       return {
            ...prev,
          name:'name should be atleast 8 characters'
       }
  ...
  data.password.length<Number(8) ?
    ...
    if (data.name.length >= Number(8) &&data.password.length >= Number(8) &&data.password === data.confirmPassword) 
  ...
  async function handleUploadedImage(e) {
    ...
    const uploadedOnCloudinary = await UploadImage(file);
    setdata((prev) => {
      return {
        ...prev,
        profilePic:uploadedOnCloudinary.url,
      };
    });
    console.log(data);
  }
    ...
    <section id="signup">
      <div className="mx-auto container p-4 mt-3">
        <div className=" p-2 py-7 w-full max-w-xl mx-auto rounded-xl bg-blue-50">
          <div className="w-20 h-20 mx-auto relative overflow-hidden rounded-full bg-black">
            <div>
              <img
                src={data?.profilePic || Loginlogo}
                alt="login-form"
            ...
            <form>
              <label>
                <div className="text-xs bg-black  text-white cursor-pointer text-center pb-4 pt-2 absolute bottom-0 w-full">
                  Upload Photo
                </div>
                <input
                  type="file"
          ...
          <form className="pt-3" onSubmit={handleSubmit}>
            <div className=" p-1">
              ...
              <p className="text-red-500 mb-1 ml-2 ">{err.name && err.name}</p>
            </div>
            <div className=" p-1">
              <label className="text-red-500 text-2xl font-medium">
                Email :{" "}
...
{/*            
            ...
            <button
              className="bg-black text-white px-6 py-3 w-full max-w-[180px] rounded-full hover:scale-110 transition-all text-2xl mx-auto block mt-2"
              type="submit"
            >
              Sign Up
            </button>
